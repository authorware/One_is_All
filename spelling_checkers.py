...
def edit_1(word, words_dict):
    ...
    if len(c):
        print('在编辑距离1中，寻找写错的w所对应的所有正确的c如下：')
        print(c)
        print('共:%d个' % len(c))
        return c
    else:
...
def edit_2(word, words_dict):
    ...
    if len(c):
        print('在编辑距离2中，寻找写错的w所对应的所有正确的c如下：')
        print(c)
        print('共:%d个' % len(c))
        return c
    else:
...
def correct(word, words_dict):
    ...
    print('在correct函数中,查找c在先验中出现的次数，也就是概率如下:')
...
if __name__ == '__main__':
    ...
    reco = correct('text', words_dict)  # 'lear'是待检查的英文字符
    print(reco)
